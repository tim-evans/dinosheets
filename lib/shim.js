import { camelize, dasherize, cssText, toCSS, forEach, keys, reduce, map, trim } from 'dinosheets/utils';
...
class StyleSheetShim {
  ...
  canApplyRule(rule, value) {
    if (supportedCssRules.hasOwnProperty(rule)) {
      return supportedCssRules[rule];
    }

    const styleSheet = testerSheet();
    const selector = '#dinosheet---tester---selector';
    const css = `${dasherize(rule)}: ${value};`;
    ...
    if (cssText(rules[0]).length > selector.length + css.length) {
  ...
  insertRule(selector, rules, index) {
    ...
    rules = reduce(keys(rules), (E, rule) => {
      ...
      if (this.canApplyRule(rule, value)) {
        E[rule] = value;
      }
      return E;
    }, {});
